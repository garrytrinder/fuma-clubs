...
export async function createResult(
  ...
  try {
    ...
    const matchResult = {
      ...
      homeTeamHalfTimeScore:
        parseInt(formData.get("halfTimeHomeGoals") as string) || 0,
      awayTeamHalfTimeScore:
        parseInt(formData.get("halfTimeAwayGoals") as string) || 0,
      ...
      homeTeamFormationId: parseInt(
        formData.get("homeTeamFormationId") as string
      ),
      awayTeamFormationId: parseInt(
        formData.get("awayTeamFormationId") as string
      ),
      penalties: false,
  ...
  const playerPerformances = JSON.parse(
    formData.get("playerPerformances") as string
  ) as PlayerPerformance[];
  const manOfTheMatch = JSON.parse(
    formData.get("motm") as string
  ) as PlayerPerformance[];
  const goalKeeperCleanSheets = JSON.parse(
    formData.get("gkCleanSheets") as string
  ) as PlayerPerformance[];
  ...
  try {
    ...
    const playerPerformance = playerPerformances.map((playerPerformance) => {
      const { goals, assists, ownGoals, teamId, playerId, positionId } =
        playerPerformance;
      return {
        resultId: result.id,
        teamId,
        ...
        manOfTheMatch: manOfTheMatch?.some(
          (motm) => motm.playerId === playerPerformance.playerId
        ),
        cleanSheet: goalKeeperCleanSheets?.some(
          (goalkeeper) => goalkeeper.playerId === playerPerformance.playerId
        ),
    ...
    return {
      message: `There was an error creating the player performances - ${
  ...
  try {
    ...
    playerPerformances.map(async (playerPerformance) => {
      ...
      if (goals > 0) {
        console.log(
          `Player (${playerId}) scored ${goals} goals for team (${teamId})`
        );
      ...
      if (assists > 0) {
        console.log(
          `Player (${playerId}) assisted ${assists} goals for team (${teamId})`
        );
      ...
      if (ownGoals > 0) {
        console.log(
          `Player (${playerId}) scored ${ownGoals} own goals for team (${teamId})`
        );
