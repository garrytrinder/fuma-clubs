...
export const { handlers, auth, signIn, signOut } = NextAuth({
  ...
  providers: [
    Discord({
      authorization:
        "https://discord.com/api/oauth2/authorize?scope=identify+openid+guilds+guilds.members.read",
  ...
  callbacks: {
    signIn: async ({ user, account, profile }) => {
      ...
      const isMember = guilds.some(
        (guild: Guild) => guild.id === process.env.DISCORD_GUILD_ID
      );
    ...
    session: async ({ session, user }) => {
      ...
      session.user.isAdmin = isAdmin ? true : false;
